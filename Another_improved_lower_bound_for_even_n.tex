...
\begin{thm}\label{thm:main_unifom_even_n_1}
...
\]Define \(g(\theta):=\mathrm{tri}(3\theta)\). For $x,y\in\mathbb R$ define the circular distance
\[
...
% \item $|g(\theta)-g(\phi)|\le L \cdot d(\theta,\phi)$, where $L:= \frac{6}{\pi}$. [But in this case $\theta-\phi$ may not in $[0, 2\pi]$]
% \end{enumerate}
% \end{lem}
...
\begin{enumerate}
\item $|g(\theta)|\le 1$ for all $\theta\in\mathbb R$.
\item $g$ is even: $g(-\theta)=g(\theta)$.
\item $g$ is $\pi$-antiperiodic: $g(\theta+\pi)=-g(\theta)$.
\item $g$ is Lipschitz on the circle: for all $\theta,\phi\in\mathbb R$,
\[
...
For (4), note that $\mathrm{tri}$ is the $2\pi$-periodic extension of the piecewise linear function
$1-\frac{2}{\pi}|x|$ on $[-\pi,\pi]$, hence $\mathrm{tri}$ is globally Lipschitz on $\mathbb R$ with
constant $2/\pi$. Therefore $g(\theta)=\mathrm{tri}(3\theta)$ is globally Lipschitz on $\mathbb R$
(with respect to the usual distance) with constant $3\cdot(2/\pi)=6/\pi$:
\[
|g(u)-g(v)|\le \frac{6}{\pi}|u-v|\qquad (u,v\in\mathbb R).
\]
Now fix $\theta,\phi\in\mathbb R$ and choose $k\in\mathbb Z$ such that
$|\theta-(\phi+2\pi k)|=d(\theta,\phi)$. Using $2\pi$-periodicity of $g$,
\[
|g(\theta)-g(\phi)|=|g(\theta)-g(\phi+2\pi k)|
...
\subsection{The configuration for even $n$}\label{sec:even_n_conf_v1}
...
There exists $N_0$ such that for all even $n=2m\ge N_0$ the configuration $\{z_k\}_{k=0}^{n-1}$ in Section~\ref{sec:even_n_conf_v1} satisfies
\[
...
Fix $i\neq j$ and set $\beta:=d(\theta_i,\theta_j)\in(0,\pi)$. Then
\[
...
\]
with $\alpha=\tfrac{2\pi\ell}{n}$ for some integer $\ell\in\{1,2,\dots,\lfloor (n-1)/4\rfloor\}$.
By Lemma~\ref{lem:gprops}(3),
\[
g(\theta_j)=g(\theta_i+\pi-\varepsilon\alpha)=-g(\theta_i-\varepsilon\alpha).
\]
Set \(a:=g(\theta_i),\qquad b:=g(\theta_i-\varepsilon\alpha)\). Then $r_i=1+t_n a$ and $r_j=1-t_n b$. A direct expansion gives
\begin{align}
|z_i-z_j|^2-4
...
\]
Moreover, for all $\alpha\ge0$ one has $-4\sin^2\frac{\alpha}{2}\le -\alpha^2+\frac{\alpha^4}{12}$.
...
Now substitute $\alpha=\frac{2\pi\ell}{n}$ and $t_n=\frac{\pi^2}{12n}(1-\frac1n)$. We now show that the right-hand side of~\eqref{eq:hbound} is $\le 0$ for all even $n\ge 8$. Since $t_n\le \pi^2/(12n)$ and $L^2+1=(\pi^2+36)/\pi^2$, we obtain $$4t_nL\alpha = \frac{4\pi^2\ell}{n^2}\Bigl(1-\frac1n\Bigr), \qquad \frac{\alpha^4}{12} = \frac{4}{3}\frac{\pi^4\ell^4}{n^4}$$ and
\[
(L^2+1)t_n^2\alpha^2
\le
\frac{\pi^2+36}{\pi^2}\cdot \frac{\pi^4}{144n^2}\cdot \Bigl(\frac{2\pi\ell}{n}\Bigr)^2
=
\frac{\pi^4(\pi^2+36)}{36} \frac{\ell^2}{n^4}.
\]
Moreover,
\[
-\alpha^2+4t_nL\alpha
...
\]
For $n\ge 8$ we have $n^2 (2- \pi^2/12) \ge \pi^2(\pi^2+36)/36$, so indeed $E_{n,\ell}\le 0$ for all $\ell\ge 2$ whenever $n\ge 8$.

\medskip
Combining the two cases, we have $E_{n,\ell}\le 0$ for every $1\le \ell\le n/4$ once $n\ge 8$.
Hence \eqref{eq:E-nl} implies $|z_i-z_j|^2-4\le 0$, i.e.\ $|z_i-z_j|\le 2$.
\end{proof}
...
\] Then \(z_i-z_j=(\zeta_i-\zeta_j)\bigl(1+t_n\rho_{ij}\bigr)\), and therefore
...
\begin{proof}
Let $p(z)=z^n-1=\prod_{j=0}^{n-1}(z-\zeta_j)$. For each root $\zeta_i$, \(p'(\zeta_i)=n\zeta_i^{n-1}=\prod_{j\neq i}(\zeta_i-\zeta_j)\). Taking absolute values and then the product over $i$ gives
\[
\prod_{i}\prod_{j\neq i}|\zeta_i-\zeta_j|
=\prod_i |p'(\zeta_i)|
=\prod_i n = n^n.
...
For all $i\neq j$, we have $|\rho_{ij}|\le 4$.

\end{lem}

\begin{proof}
Write $g_i:=g(\theta_i)$. Then \(v_i-v_j=g_i(\zeta_i-\zeta_j)+(g_i-g_j)\zeta_j\), hence
\[
\rho_{ij}=g_i+(g_i-g_j)\frac{\zeta_j}{\zeta_i-\zeta_j}.
\]Let $\delta_{ij}:= d(\theta_i,\theta_j)\in(0,\pi]$.
Since $g$ is $2\pi$-periodic, $|g_i-g_j|\le L\delta_{ij}$ and
$|\zeta_i-\zeta_j|=2\sin(\delta_{ij}/2)$. Therefore
\[
|\rho_{ij}|
\le |g_i|+\frac{|g_i-g_j|}{|\zeta_i-\zeta_j|}
\le 1+\frac{L\delta_{ij}}{2|\sin(\delta_{ij}/2)|}.
\]
...
For $0<x\le\pi$, concavity of $\sin$ on $[0,\pi/2]$ gives $\sin(x/2)\ge \frac{x}{\pi}$, hence $|\rho_{ij}|\le 1+\frac{L\pi}{2}=1+\frac{6}{\pi}\cdot\frac{\pi}{2}=4$.
\end{proof}

\subsection{Second-order expansion of $\log(\Delta/n^n)$}
Taking logarithms on both sides of~\eqref{comb_lem_ro_fa_yi_1}, we obtain
\begin{equation}\label{eq:logsum}
\log\frac{\Delta(z_0,\dots,z_{n-1})}{n^n}
=\sum_{i\neq j}\log\bigl|1+t_n\rho_{ij}\bigr|.
\end{equation}

Let \( \Re z \) denote the real part of a complex number $z$.
...
% \rho(x,y):=\frac{f(x)-f(y)}{\xi(x)-\xi(y)}\quad(d(x,y)>0),
% \end{equation}
% and $F(x,y):=\Re(\rho(x,y)^2)$ for $x\neq y$, with $F(x,x):=0$.
% Clearly, $\rho(\theta_i,\theta_j)=\rho_{ij}$.
...
\]
For $i\neq j$ we have $d(\theta_i,\theta_j)>0$, hence $\rho(\theta_i,\theta_j)=\rho_{ij}$ and
$F(\theta_i,\theta_j)=\Re(\rho_{ij}^2)$.
...
\begin{lem}\label{lem:Riemann}
The function $F$ is bounded on $[0,2\pi]^2$ and is continuous at every point $(x,y)$ with $d(x,y)>0$.
...
Continuity holds for $d(x,y)>0$ since the denominator is nonzero.
Let
\[
D:=\{(x,y)\in[0,2\pi]^2:\ d(x,y)=0\}=\{(x,y)\in[0,2\pi]^2:\ e^{ix}=e^{iy}\}.
\]
On $[0,2\pi]^2$ one has $D=\{x=y\}\cup\{(0,2\pi),(2\pi,0)\}$, hence $D$ has Lebesgue measure $0$ and $F$ is Riemann integrable.
Let $h:=2\pi/n$ and $\theta_i:=ih$ for $i=0,1,\dots,n-1$. Consider the uniform partition of $[0,2\pi]^2$
into $n^2$ squares of side $h$ and choose the sample point $(\theta_i,\theta_j)$ in the $(i,j)$th square.
Since $F$ is Riemann integrable, these Riemann sums converge:
\[
\lim_{n\to\infty} h^2\sum_{i,j=0}^{n-1}F(\theta_i,\theta_j)
=\int_0^{2\pi}\int_0^{2\pi}F(x,y)\,dx\,dy.
\]
Dividing by $(2\pi)^2$ gives
\[
\lim_{n\to\infty}\frac{1}{n^2}\sum_{i,j=0}^{n-1}F(\theta_i,\theta_j)
=
\frac{1}{4\pi^2}\int_0^{2\pi}\int_0^{2\pi}F(x,y)\,dx\,dy.
...
Finally, since $F(\theta_i,\theta_i)=0$ for all $i$ by definition, we have
$\sum_{i,j}F(\theta_i,\theta_j)=\sum_{i\neq j}F(\theta_i,\theta_j)$, and for $i\neq j$,
$F(\theta_i,\theta_j)=\Re(\rho_{ij}^2)$. This yields the stated consequence and the definition of $J$.
\end{proof}


% \begin{proof}
% \textbf{Boundedness.}
% Since $f$ is Lipschitz on the circle, we have
% \[
% |f(x)-f(y)|\le \Lip(f)\,d(x,y)\qquad(x,y\in\mathbb R),
% \]
% where $\Lip(f):=\sup_{d(u,v)>0}\frac{|f(u)-f(v)|}{d(u,v)}<\infty$.
% If $d(x,y)>0$, then $|e^{ix}-e^{iy}|=2\sin(d(x,y)/2)\ge 2d(x,y)/\pi$, hence
% \[
% |\rho(x,y)|\le \Lip(f)\,\frac{d(x,y)}{2\sin(d(x,y)/2)}\le \frac{\pi}{2}\Lip(f).
% \]
% Therefore $|F(x,y)|\le \frac{\pi^2}{4}\Lip(f)^2$ when $d(x,y)>0$, while $F(x,y)=0$ when $d(x,y)=0$ by definition.
% Thus $F$ is bounded on $[0,2\pi]^2$.

% \textbf{Continuity off $\{d=0\}$.}
% On the set $\{(x,y):d(x,y)>0\}$ the denominator $e^{ix}-e^{iy}$ is nonzero, and both
% $f$ and $e^{ix}$ are continuous; hence $\rho$ (and therefore $F$) is continuous there.

% \textbf{Riemann integrability.}
% Let
% \[
% D:=\{(x,y)\in[0,2\pi]^2:\ d(x,y)=0\}=\{(x,y)\in[0,2\pi]^2:\ e^{ix}=e^{iy}\}.
% \]
% On $[0,2\pi]^2$ one has $D=\{x=y\}\cup\{(0,2\pi),(2\pi,0)\}$, hence $D$ has Lebesgue measure $0$.
% Thus the set of discontinuities of $F$ is contained in $D$ and has measure $0$; by Lebesgue's criterion,
% $F$ is Riemann integrable on $[0,2\pi]^2$.

% \textbf{Convergence of uniform-grid Riemann sums.}
% Let $h:=2\pi/n$ and $\theta_i:=ih$ for $i=0,1,\dots,n-1$. Consider the uniform partition of $[0,2\pi]^2$
% into $n^2$ squares of side $h$ and choose the sample point $(\theta_i,\theta_j)$ in the $(i,j)$th square.
% Since $F$ is Riemann integrable, these Riemann sums converge:
% \[
...
\]
Define the \emph{Fej\'er mean} of a $2\pi$-periodic continuous function $f$ by
\[
...
Let $f:\mathbb R\to\mathbb C$ be $2\pi$--periodic and Lipschitz. Write
\[
\Lip(f):=\sup_{d(x,y)>0}\frac{|f(x)-f(y)|}{d(x,y)}.
\] then $\Lip(f^{(N)})\le \Lip(f)$ for all $N$.
...
% &\le \frac{1}{2\pi}\int_0^{2\pi}\Lip(f)\,d(x,y)\,K_N(t)\,dt
% =\Lip(f)\,d(x,y).
% \end{align*}
% Thus $\Lip(f^{(N)})\le \Lip(f)$.
% Also $|e^{ix}-e^{iy}|=2\sin(d(x,y)/2)\ge 2 d(x,y)/\pi$, this completes the proof.
% \end{proof}

\begin{proof}
Since $K_N$ is a probability kernel, and $d(x-t,y-t)=d(x,y)$ for all $t$,
\begin{align*}
|f^{(N)}(x)-f^{(N)}(y)|
&=\left|\frac{1}{2\pi}\int_0^{2\pi}\bigl(f(x-t)-f(y-t)\bigr)K_N(t)\,dt\right|\\
&\le \frac{1}{2\pi}\int_0^{2\pi}|f(x-t)-f(y-t)|\,K_N(t)\,dt\\
&\le \frac{1}{2\pi}\int_0^{2\pi}\Lip(f)\,d(x-t,y-t)\,K_N(t)\,dt\\
&=\Lip(f)\,d(x,y).
\end{align*}
Thus $\Lip(f^{(N)})\le \Lip(f)$.

If $d(x,y)>0$, then $|e^{ix}-e^{iy}|=2\sin(d(x,y)/2)\ge 2d(x,y)/\pi$,
and hence
\[
\left|\frac{f^{(N)}(x)-f^{(N)}(y)}{e^{ix}-e^{iy}}\right|
\le \Lip(f)\,\frac{d(x,y)}{2\sin(d(x,y)/2)}
\le \frac{\pi}{2}\Lip(f).
\qedhere\]
\end{proof}
...
\[
J=\frac{1}{4\pi^2}\int_0^{2\pi}\int_0^{2\pi} \Re(\rho^2)=\Re B(f)=B(f).
\]
